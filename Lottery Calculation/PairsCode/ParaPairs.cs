...
namespace LotterySharper.LotteryCalculation.PairsCode
{
    public class ParaPairs : IParaPairs
    {
        ...
        public void FindPairsParallel(string lotteryName,
                                      (IEnumerable<int[]> AllNumbers, IEnumerable<int> DistinctNumbers, IEnumerable<int> BonusNumbers) parsedLotto)
        {
            ...
            List<IPairs> pairs = (from firstNum in parsedLotto.DistinctNumbers
                ...
                select new Pairs
                { First = firstNum, Second = secondNum }).Cast<IPairs>()
                .ToList();
